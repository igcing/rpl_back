...
async function createTableBanco(){
    ...
    return {results: 'createTableBanco'};
}
...
async function createTableTipoCuenta(){
    ...
    return {results: 'createTableTipoCuenta'};
}
...
async function createTablePersona(){
    ...
    "   email_persona character varying(200) NOT NULL,"+
    "   id_tipo_cuenta integer NOT NULL,"+
    "   id_banco integer NOT NULL,"+
    "   CONSTRAINT pk_persona PRIMARY KEY (id_persona),"+
    "   CONSTRAINT uk_persona UNIQUE (rut_persona, id_tipo_cuenta, id_banco, numero_cuenta),"+
    ...
    return {results: 'createTablePersona'};
}

async function creatTableTransferencia() {
    let query = "CREATE TABLE IF NOT EXISTS public.transferencia ("+
        " id_transferencia integer NOT NULL GENERATED ALWAYS AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 9999999 ),"+
        ...
        "     NOT VALID)";
    await db.query(query).catch(err => {console.log(err)});
    return {results: 'creatTableTransferencia'};
}
